...
import { createBook, deleteBook, getBooks, updateBook } from '../api/services/productsService.js';
...
export default function AdminBooks() {
  ...
  const load = useCallback(async (p = page, l = limit) => {
    try {
      const data = await getBooks({ page: p, limit: l });
      const items = data.items || data;
      setBooks(items);
      setTotal(data.total || (items && items.length) || 0);
      setPage(data.page || p);
  ...
  const addBook = async (e) => {
    ...
    try {
      await createBook({ title: form.title, author: form.author, price: Number(form.price) || 0, image: form.image || '/assets/placeholder.png', description: form.description, stock: Number(form.stock) || 0 }, token);
  ...
  const saveEdit = async (id) => {
    if (!token) return window.location.href = '/login';
    try {
      await updateBook(id, { stock: Number(editingStock) }, token);
      cancelEdit();
      load(page, limit);
