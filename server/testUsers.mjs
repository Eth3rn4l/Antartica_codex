...
(async ()=>{
  try{
    ...
    const users = await fetch('http://localhost:4000/api/users?page=1&limit=10',{headers:{Authorization:`Bearer ${token}`}});
    const udata = await users.json();
    console.log('users status', users.status, JSON.stringify(udata, null, 2));
